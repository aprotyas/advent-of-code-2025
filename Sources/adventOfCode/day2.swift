...
func day2() {
  ...
  guard
    let input = try? String(contentsOf: inputURL, encoding: .utf8).trimmingCharacters(
  ...
  let idRanges = input.map { $0.split(separator: "-").compactMap { Int($0) } }.filter {
    $0.count == 2
  ...
  let part1 = idRanges.reduce(into: 0) { invalidIDSum, currentID in
    ...
    let (leftHalf, rightHalf) = currentID.quotientAndRemainder(
      dividingBy: Int(pow(Double(10), Double(smallestUnit / 2 + 1))))
  ...
  let part2 = idRanges.reduce(into: 0) { invalidIDSum, currentID in
    ...
    for unit in unitsToTest {
      ...
      // I got lazy.
      if pieces.count > 1 && Set(pieces).count == 1
        && Int(pieces.map { String($0) }.joined()) == currentID
      {
        invalidIDSum += currentID
        return
      }
